...
<body class="gray-bg">
        ...
        <form id="search_from">
			  <div class="col-lg-2">
				<div class="input-group">
			      ...
			      <input type="text" name="experssName" class="form-control" id="txt_search_experssName" >
				</div>
			  </div>
			  <div class="col-lg-2">
				<div class="input-group">
					<span class="input-group-addon">所属公司</span>
					<select class="form-control" name="expressTypeId" id = "txt_search_id">
				...
				<div class="input-group">
					<span class="input-group-addon">入职时间区间</span>
						<input placeholder="开始时间" id="txt_search_startDate" name="start" class="laydate-icon form-control layer-date"/>
                     	<input placeholder="结束时间"id="txt_search_endDate" name="end" class="laydate-icon form-control layer-date"/>
				</div>
					...
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>名字：</td>
								<td>
									<input type="text" id="experssName" name="experssName">
								</td>						
							</tr>
							...
							<tr>
								<td>特点：</td>
								<td>
									<input type="checkbox" id="experssTrait" name="experssTrait" value="听歌">听歌
									<input type="checkbox" id="experssTrait" name="experssTrait" value="看视频">看视频
									<input type="checkbox" id="experssTrait" name="experssTrait" value="跑酷">跑酷
								</td>						
							</tr>
							...
							<tr>
								<td>入职时间：</td>
								<td>
									<input placeholder="入职时间" id="experssTime" name="experssTime" class="laydate-icon form-control layer-date"/>									
								</td>						
							</tr>
							...
							<tr>
								<td>所属公司：</td>
								<td colspan="4">
									<select class="form-control" name="expressTypeId" id = "expressTypeId1" aria-required="true" required>
	...
	<div class="modal fade" id="modal_user_import" role="dialog" aria-labelledby="modal_user_import" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_import" method="post" action="importFile.htm" enctype="multipart/form-data">						
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>请选择文件：</td>
								<td>
	...
	<!-- 统计(柱状图)对话框 -->
	<div class="modal fade" id="modal_user_tongji" role="dialog" aria-labelledby="modal_user_tongji" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
	...
	<!-- 统计(饼状图)对话框 -->
	<div class="modal fade" id="modal_user_tongji1" role="dialog" aria-labelledby="modal_user_tongji1" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
	...
	Date.prototype.Format = function (fmt) {
	  ...
	  //外部js调用
    laydate({
        elem: '#txt_search_startDate', //目标元素。由于laydate.js封装了一个轻量级的选择器引擎，因此elem还允许你传入class、tag但必须按照这种方式 '#id .class'
	...
	var TableInit = function () {
	    ...
	    oTableInit.queryParams = function (params) {
	        var temp = {//这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
	            ...
	            experssName: $("#txt_search_experssName").val(),
	            expressTypeId: $("#txt_search_id").val(),
	            start: $("#txt_search_startDate").val(),
	            end: $("#txt_search_endDate").val(),	          
	    	 ...
	    	 $.ajax({
	    		    ...
	    		    success:function(res){	    		    	
	    		    	...
	    		    	for(var i =0; i<datas.length;i++){
		    	    		...
		    	    		$('#modal_user_tongji').modal({backdrop: 'static', keyboard: false});
		    				$('#modal_user_tongji').modal('show');
	    	...
	    	$("#btn_tongji1").click(function(){
	    	 $.ajax({
	    		    ...
	    		    success:function(res){	    		    	
	    		    	...
	    		    	for(var i =0; i<datas.length;i++){
	    		    		var c = datas[i];
		    		    	xArray1.push(c.cname);
		    		    	var obj  = {value:c.num, name:c.cname};
		    		    	yArray1.push(obj);
		    	    		...
		    	    		$('#modal_user_tongji1').modal({backdrop: 'static', keyboard: false});
		    				$('#modal_user_tongji1').modal('show');
	...
	function initEditUser(getSelection){
		...
		$('#experssName').val(getSelection.experssName);
		$("input[type='radio'][value='"+getSelection.sex+"']").prop("checked",true);
		var experssTrait = getSelection.experssTrait.split(",");
		for(var i =0;i<experssTrait.length;i++){
			var b = experssTrait[i];
			$("input[type='checkbox'][value='"+b+"']").prop("checked",true);
		}
		$('#experssTime').val(new Date(getSelection.experssTime).Format('yyyy-MM-dd'));		
		$('#expressTypeId1').val(getSelection.pid);
