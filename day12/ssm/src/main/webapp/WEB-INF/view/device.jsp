...
<body class="gray-bg">
        ...
        <div class="row">
			  <div class="col-lg-2">
				<div class="input-group">
			      ...
			      <input type="text" name="devicename" class="form-control" id="txt_search_name" >
				</div>
			  </div>
			  <div class="col-lg-2">
				<div class="input-group">
					<span class="input-group-addon">设备状态</span>
					<select class="form-control" name="status" id = "txt_search_devicetypeid">
				...
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>设备名称：</td>
								<td><input type="text" id="devicename" name="devicename" class="form-control" aria-required="true" required/>
								</td>
							</tr>	
							
							<tr>
								<td>内存：</td>
								<td><input type="text" id="deviceram" name="deviceram" class="form-control" aria-required="true" required/>
								</td>
							</tr>	
							
							<tr>
								<td>设备价格：</td>
								<td><input type="text" id="price" name="price" class="form-control" aria-required="true" required/>
								</td>
							</tr>	
							
							<tr>
								<td>机身颜色：</td>
								<td><input type="text" id="color" name="color" class="form-control" aria-required="true" required/>
								</td>
							</tr>	
							...
							<tr>
								...
								<td colspan="4">
									<select class="form-control" name="devicetypeid" id = "devicetypeid" aria-required="true" required>
										<option value="">---请选择---</option>
										<c:forEach items="${DeviceStatus }" var="r">
	...
	<!-- 添加设备类别名称对话框 -->
	<div class="modal fade" id="modal_user_addType" role="dialog" aria-labelledby="modal_user_addType" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_addType" method="post" action="addDeviceType.htm">
						<input type="hidden" name="devicetypeid" id="hidden_txt_userid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>设备名称：</td>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_addType">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	$(function () {
	    ...
	    var validator1 = $("#form_addType").validate({
    		submitHandler: function(form){
   		      $(form).ajaxSubmit({
   		    	dataType:"json",
   		    	success: function (data) {
	    ...
	    $("#submit_form_addType").click(function(){
	    	$("#form_addType").submit();
	    });
	...
	var TableInit = function () {
	    ...
	    oTableInit.queryParams = function (params) {
	        var temp = {//这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
	            ...
	            devicename: $("#txt_search_name").val(),
	            status: $("#txt_search_devicetypeid").val(),
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_addType").click(function(){
	    		$('#modal_user_addType').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_addType').modal('show');
	        });
