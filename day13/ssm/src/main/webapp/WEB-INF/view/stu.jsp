...
<body class="gray-bg">
					...
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								...
								<td>&nbsp;&nbsp;</td>
								<td>学生性别：</td>
								<td><input type="radio" id="sSex" name="sSex" value="男"
								aria-required="true" required/>男
								<input type="radio" id="sSex" name="sSex" value="女"
							...
							<td>学生爱好：</td>
								<td>
									...
									<input type="checkbox" id="sHobby2" name="sHobby"  aria-required="true" required value="足球"/>足球
									<input type="checkbox" id="sHobby3" name="sHobby"  aria-required="true" required value="排球"/>排球
								</td>
								<td>专业：</td>
								<td colspan="4">
							...
							<tr>
								<td>学生生日：</td>
								<td><input placeholder="学生生日" id="sBirth" name="sBirth" class="laydate-icon form-control layer-date"/></td>
								<td>&nbsp;&nbsp;</td>
    ...
    //外部js调用
    laydate({
        elem: '#sBirth', //目标元素。由于laydate.js封装了一个轻量级的选择器引擎，因此elem还允许你传入class、tag但必须按照这种方式 '#id .class'
        event: 'focus', //响应事件。如果没有传入event，则按照默认的click
        format: 'YYYY-MM-DD'// 日期格式
    });
    			    ...
    			    success:function(res){
    			    	...
    			    	if(res.success){
    		    			...
    		    			res.elist.forEach(function(obj){
    		    				xdata.push(obj.mname);
    		    				ydata.push({value:obj.num,name:obj.mname});
    		    			});
    		    			//设置图表  饼状图
    		    			option = {
	    			    		    title: {
	...
	function initEditUser(getSelection){
		...
		$("input[value='"+getSelection.sSex+"']").prop("checked",true);//性别   
